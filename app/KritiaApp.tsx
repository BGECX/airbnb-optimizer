...
const demoCompany: RecordValue = { raisonSociale: "KRITIA Démonstration", siret: "", siren: "", tvaIntra: "", adresse: "9 Grande Rue", codePostal: "51170", ville: "Brouillet", telephone: "", email: "demonstration@getkritia.com", siteWeb: "https://www.getkritia.com", logoUrl: "", couleurPrimary: "#2563eb", couleurSecondary: "#f59e0b" };

function CompanySettings({ demo, request, back }: { demo: boolean; request: ApiRequest; back: () => void }) {
  const [tab, setTab] = useState<"company" | "numbering" | "vat">("company");
  ...
  const unavailable = ["Assurances · CGV", "Horaires", "Marges", "Comptes bancaires", "Autres paramètres", "Attestation"];
  return <section className="settings-page company-settings"><div className="company-heading"><button type="button" className="secondary compact" onClick={back}>← Retour aux paramètres</button><div><p className="eyebrow">PARAMÈTRES · ENTREPRISE</p><h2>Mon entreprise</h2></div></div><nav className="company-tabs" aria-label="Paramètres de l’entreprise"><button type="button" className={tab === "company" ? "active" : ""} onClick={() => setTab("company")}>Mon entreprise</button><button type="button" className={tab === "numbering" ? "active" : ""} onClick={() => setTab("numbering")}>Numérotation</button><button type="button" className={tab === "vat" ? "active" : ""} onClick={() => setTab("vat")}>TVA</button>{unavailable.map((item) => <button type="button" disabled key={item}>{item}<small>À venir</small></button>)}</nav>{error && <div className="form-error settings-feedback">{error}</div>}{message && <div className="form-notice settings-feedback">{message}</div>}{tab === "company" && (loading ? <div className="panel">Chargement de la fiche entreprise…</div> : <form className="company-form" onSubmit={save}><div className="company-form-grid"><section><h3>▣ Informations générales</h3><div className="company-fields">{fields.slice(0, 4).map(([key, title, placeholder]) => <label key={key}>{title}{key === "raisonSociale" && <b>*</b>}<input value={String(company[key] ?? "")} onChange={(event) => change(key, event.target.value)} placeholder={placeholder} required={key === "raisonSociale"} /></label>)}</div></section><section><h3>⌂ Coordonnées</h3><div className="company-fields two-columns">{fields.slice(4, 9).map(([key, title, placeholder]) => <label className={key === "adresse" ? "full" : ""} key={key}>{title}<input type={key === "email" ? "email" : "text"} value={String(company[key] ?? "")} onChange={(event) => change(key, event.target.value)} placeholder={placeholder} /></label>)}</div></section><section><h3>◇ Identité visuelle</h3><div className="company-fields">{fields.slice(9).map(([key, title, placeholder]) => <label key={key}>{title}<input type="url" value={String(company[key] ?? "")} onChange={(event) => change(key, event.target.value)} placeholder={placeholder} /></label>)}<div className="color-fields"><label>Couleur principale<input type="color" value={String(company.couleurPrimary ?? "#2563eb")} onChange={(event) => change("couleurPrimary", event.target.value)} /></label><label>Couleur secondaire<input type="color" value={String(company.couleurSecondary ?? "#f59e0b")} onChange={(event) => change("couleurSecondary", event.target.value)} /></label></div></div></section></div><div className="company-save"><span>{demo ? "Aucun changement de démonstration ne sera conservé." : "Modification réservée aux administrateurs."}</span><button className="primary compact" disabled={saving}>{saving ? "Enregistrement…" : "✓ Enregistrer"}</button></div></form>)}{tab === "numbering" && <section className="parameter-workspace"><div className="parameter-intro"><h3>Numérotation des documents</h3><p>Définissez les préfixes et formats utilisés pour les prochains documents.</p></div><div className="numbering-list">{numbering.length ? numbering.map((item, index) => <article key={String(item.type)}><div><strong>{label(item.type)}</strong><small>Numéro actuel : {String(item.numeroActuel ?? 0)}</small></div><label>Préfixe<input value={String(item.prefixe ?? "")} onChange={(event) => setNumbering((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, prefixe: event.target.value } : row))} /></label><label>Format<input value={String(item.format ?? "")} onChange={(event) => setNumbering((rows) => rows.map((row, rowIndex) => rowIndex === index ? { ...row, format: event.target.value } : row))} /></label><button type="button" className="secondary compact" onClick={() => void saveNumbering(item)}>Enregistrer</button></article>) : <div className="empty"><h3>Aucune numérotation configurée</h3><p>Les lignes apparaîtront après leur initialisation côté serveur.</p></div>}</div></section>}{tab === "vat" && <section className="parameter-workspace"><div className="parameter-intro"><h3>Taux de TVA</h3><p>Consultez les taux disponibles et ajoutez ceux nécessaires à votre activité.</p></div><div className="vat-layout"><div className="vat-list">{vatRates.map((item) => <article key={String(item.id)}><strong>{Number(item.taux).toLocaleString("fr-FR")} %</strong><span>{String(item.nom)}</span>{Boolean(item.isDefault) && <em>Par défaut</em>}</article>)}</div><form className="vat-form" onSubmit={addVat}><h3>Ajouter un taux</h3><label>Intitulé<input name="nom" required placeholder="Ex. Taux réduit rénovation" /></label><label>Taux (%)<input name="taux" type="number" min="0" max="100" step="0.01" required /></label><label className="checkbox-line"><input name="isDefault" type="checkbox" /> Utiliser par défaut</label><button className="primary compact">＋ Ajouter le taux</button></form></div></section>}</section>;
}
