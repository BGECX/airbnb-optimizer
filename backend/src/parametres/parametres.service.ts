...
export class ParametresService {
  ...
  async updateEntreprise(data: UpdateEntrepriseDto) {
    if (data.siret) {
      ...
      if (!/^\d{14}$/.test(siret) || !this.isLuhnValid(siret)) throw new BadRequestException('Le numéro SIRET est invalide');
      data.siret = siret;
    ...
    if (data.tvaIntra) {
      ...
      if (!result.valid) throw new BadRequestException('Le numéro de TVA intracommunautaire est invalide selon VIES');
      data.tvaIntra = vat;
    }
  ...
  async verifyTVA(numero: string) {
    ...
    if (!/^[A-Z]{2}[A-Z0-9]{2,14}$/.test(normalized)) throw new BadRequestException('Format du numéro de TVA invalide');
    try {
      const response = await fetch('https://ec.europa.eu/taxation_customs/vies/rest-api/check-vat-number', {
        method: 'POST',
        headers: { 'content-type': 'application/json', accept: 'application/json', 'user-agent': 'KRITIA-BTP/1.0 (https://getkritia.com)' },
        body: JSON.stringify({ countryCode: normalized.slice(0, 2), vatNumber: normalized.slice(2) }),
        signal: AbortSignal.timeout(8000),
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const result = await response.json() as Record<string, unknown>;
      return { numero: normalized, valid: result.valid === true, nom: result.name ?? null, adresse: result.address ?? null, dateVerification: result.requestDate ?? new Date().toISOString() };
    } catch (error) {
      if (error instanceof BadRequestException) throw error;
      throw new BadGatewayException('Le service européen VIES est momentanément indisponible');
    }
  }
  ...
  private async verifySiretExists(siret: string) {
    try {
      const response = await fetch(`https://recherche-entreprises.api.gouv.fr/search?q=${siret}&per_page=1`, { headers: { accept: 'application/json', 'user-agent': 'KRITIA-BTP/1.0 (https://getkritia.com)' }, signal: AbortSignal.timeout(6000) });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const payload = await response.json() as { results?: Array<{ siege?: { siret?: string }; matching_etablissements?: Array<{ siret?: string }> }> };
      const found = payload.results?.some((company) => company.siege?.siret === siret || company.matching_etablissements?.some((item) => item.siret === siret));
      if (!found) throw new BadRequestException('Ce SIRET est introuvable dans le registre public');
    } catch (error) {
      if (error instanceof BadRequestException) throw error;
